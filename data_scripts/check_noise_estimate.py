...
if __name__ == '__main__':
    # im = imread('/DATA/wangshen_data/CODES/low-lit-enhance/results/test_40_histmatch/00000.png')
    # im = img_as_float(im)
    # est_level = noise_estimate(im, 4)
    # print(f"Estimated Sigma: {est_level * 255:6.4f}")
    Output_PATH = '/DATA/wangshen_data/ShortLongDataset/Combined_Dataset'
    GO_Other_DATA = "/DATA/wangshen_data/ShortLongDataset/Combined_Dataset/test_noise_random"
    GO_Other_GT = Output_PATH + '/'  + "test_sharp"
    IMG_LIST = Output_PATH + '/' + "test_list"

    subdir = sorted(os.listdir(GO_Other_GT))
    ...
    for dir in subdir:
        ...
        for index, its in enumerate(subfolders):
            ...
            im = imread(b1)
            im = img_as_float(im)
            est_level = noise_estimate(im, 4) * 255.0
            var = (est_level/255.0)**2
            st = dir+'/'+subfolder+'/'+str(var)+'/'+str(est_level)
            img_list_all.append(st)
            print('{}'.format(st))
        f1.close()

    fl = open(os.path.join(Output_PATH, "estimated_noise_str.txt"), 'w')
    sep = '\n'
    fl.write(sep.join(img_list_all))
    fl.close()
