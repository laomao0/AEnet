...
if __name__ == '__main__':
    ...
    Output_PATH = '/DATA/wangshen_data/CODES/low-lit-enhance/results'
    Name = 'test_21_star_histmatch'
    GO_Other_DATA = os.path.join(Output_PATH, Name)

    imgs = sorted(os.listdir(GO_Other_DATA))

    img_list_all = []

    for it in imgs:
        b1 = os.path.join(GO_Other_DATA, it)
        im = imread(b1)
        im = img_as_float(im)
        est_level = noise_estimate(im, 4) * 255.0
        var = (est_level/255.0)**2
        st = it+'/'+str(var)+'/'+str(est_level)
        img_list_all.append(st)
        print('{}'.format(st))

    fl = open(os.path.join(Output_PATH, Name+"estimated_noise_str.txt"), 'w')
    sep = '\n'
