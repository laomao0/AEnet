...
def ShortLong_loader(root, im_path, output_root, img_list):
    ...
    for i in li:
        ...
        noise_level_img = math.sqrt(noise_var)*255.0
        if not os.path.exists(output_path):
            img_short = imread(path_short_path)
            # img_short_noisy = add_noise(img_short, var=noise_var)
            img_short = np.float32(img_short/255.)
            img_short_noisy = img_short + np.random.normal(0, noise_level_img/255., img_short.shape)
            img_short_noisy = (img_short_noisy*255.0).round().clip(0,255).astype('uint8')
...
out_root = '/DATA/wangshen_data/ShortLongDataset/Combined_Dataset/test_noise_random_numpy_2'
...
WRIList = []
for it in test_list:
    WRIList = ShortLong_loader(root, it, out_root, WRIList)

fl = open(os.path.join(out_root, "noise_str_numpy.txt"), 'w')
sep = '\n'
