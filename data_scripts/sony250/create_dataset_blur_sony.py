...
def main():
    ...
    if (args.dataset == "sony240fps_blur"):
        # f = open("adobe240fps/test_list.txt", "r")
        #f = open(args.dataset[:-5] + '/test_list.txt', "r")
        f = open('/DATA/wangshen_data/CODES/low-lit-enhance/data_scripts/sony250/sony_test_list.txt', "r" )
        if debug == True:
            videos = [f.read().split('\n')[0]]
        else:
            videos = f.read().split('\n')
        ...
        if args.enable_train == 1:
            print("train")
            f = open('/DATA/wangshen_data/CODES/low-lit-enhance/data_scripts/sony250/sony_train_list.txt', "r")
            videos = f.read().split('\n')
            # videos = ['2C0093.MP4']
            if debug == True:
                videos = [videos[0]]
